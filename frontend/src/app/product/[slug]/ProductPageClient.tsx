...
function ImageGallery({ images, categoryName, productName }: { images: string[]; categoryName?: string; productName: string }) {
        ...
        <div className="mt-3 flex gap-2">
          {review.images.map((img, i) => (
            <div key={i} className="w-16 h-16 rounded-lg overflow-hidden bg-surface-tertiary">
              <Image src={img} alt={`Photo from ${review.user.name}'s review`} width={64} height={64} className="object-cover w-full h-full" />
            </div>
          ))}
        </div>
      )}
      ...
      <Container className="py-6 lg:py-8">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
          <div className="min-w-0 lg:sticky lg:top-24 lg:self-start">
            <FadeView>
              <ImageGallery images={effectiveImages} categoryName={product.category?.name} productName={product.name} />
            </FadeView>
          </div>
